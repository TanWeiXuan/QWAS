...
from stability_assist.weights import load_actor
...
def main() -> None:
    ...
    source.add_argument("--model", type=Path, help="portable .qwasmlp model")
    ...
    if args.checkpoint:
        ...
        actor = model.actor
        source_label = f"checkpoint={args.checkpoint}"
    else:
        actor = load_actor(args.model).to(device)
        source_label = f"model={args.model}"
    actor.eval()
    print(f"Evaluation start: {source_label}, seeds={len(args.seeds)}, device={device}", flush=True)
    metrics = evaluate_actor(actor, args.seeds, device, args.duration, include_all_modes=True)
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(metrics, indent=2) + "\n", encoding="utf-8")
    for mode in ("no_assist", "pd_only", "zero", "trained"):
        result = metrics[mode]
    ...
    for mode in ("no_assist", "pd_only", "zero", "trained"):
        recovery = metrics["release_recovery"][mode]
        print(f"release {mode}: recovery={recovery['recovery_success_rate']:.3f}, "
              f"median_settle={recovery['median_settling_time_seconds']:.3f}s, "
              f"p90={recovery['p90_settling_time_seconds']:.3f}s, "
              f"final_tilt={recovery['mean_final_tilt_degrees']:.2f}deg, "
              f"final_rate={recovery['mean_final_pitch_roll_rate']:.3f}", flush=True)
